...
from src.SamModel import SamModel
...
class FastSamWrapper(SamModel):
    ...
    def segmentFrame(self, image):
        ...
        everything_results = self.fastSamModel(image, device=self.device, retina_masks=True, imgsz=1080, conf=self.conf, iou=self.iou,)
